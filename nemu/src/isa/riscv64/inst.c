...
//  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = BITS(src1 + src2,63,0)); //li,mv
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(src1>=src2)  s->dnpc = dest + s->pc );
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if((unsigned)src1 < (unsigned)src2) R(dest) = 1 ); //seqz
  //INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS((BITS(src1,31,0) + BITS(src2,31,0)),31,0), 32) );
