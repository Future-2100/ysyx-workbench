...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
 ...
 // INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + src2) ; R(dest) = s->pc + 4); //ret
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, if(src1 < src2) R(dest) = 1 );
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, if(src1 < src2) R(dest) = 1 ); 
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, if((unsigned)src1 < (unsigned)src2) R(dest) = 1 );
